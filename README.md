# 👩‍💻 Shereen Hassan Sayed

🎯 **Junior Front-End Developer**  
Passionate about building clean, responsive, and user-friendly web interfaces.

---

## 👩‍🎓 About Me
- 🌍 Based in Egypt 🇪🇬  
- 🏫 Faculty of Arts – Helwan University  
- 💡 Self-learner, building my skills step by step  
- 🎯 Goal: Become a professional software engineer  

---

## 🛠️ Skills & Technologies

### Front-End
- **HTML** – Basics & small projects ✅  
- **CSS** – Basics & responsive layouts ✅  
- **JavaScript** – Currently learning 🟡  
- **React** – Learning fundamentals 🟡  

### Programming
- **Python** – Basics (variables, loops, functions) + small projects ✅  
- **Java** – Data Structures (no projects yet) 🟡  

---

## 📚 Current Learning Path
- Strengthen **JavaScript fundamentals**  
- Learn **React** step by step  
- Build more **frontend projects**  
- Practice **Python** with small applications  

---

## 🌐 Social Accounts
- 📘 Facebook: [shery.hassan.9699](https://www.facebook.com/shery.hassan.9699)
- ✉️ Email: [sheryhassan1991@gmail.com](mailto:sheryhassan1991@gmail.com)

---

## 🤝 Open To
- Learning opportunities  
- Beginner-friendly collaborations  
- Front-end practice projects  

---

⭐ *Always learning. Always improving.*
